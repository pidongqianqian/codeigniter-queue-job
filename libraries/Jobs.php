...
class Jobs {
	 ...
	 * @access  public
	 * @param   string $queue Name of the queue
	 * @param   string $data Data to remove
	 * @return  bool 1 if ok, 0 if not
	 */
	...
	private function dequeue($queue) {
		...
		$data = $this -> _ci -> redis -> blpop($list_queues);
		// Check if exists
		if (!$data) {
			return FALSE;
		}
    ...
    public function doWorker($worker_name = 'worker', $queues, $interval = null)
    {
    ...
    /**
     * Judge whether there are jobs in redis by the given queue name
     *
     * @access  private
     * @param   string $queue Name of the queue
     * @return  bool
     */
    public function jobExistInRedis($queue) {
        $exist = false;
        // Check if queues is an array
        if (is_array($queue)) {
            // Prefix elements from array
            foreach ($queue as &$value) {
                $num = $this->get_queue_size($value);
                if ($num > 0) {
                    $exist = true;
                }
            }
        } else {
            $num = $this->get_queue_size($queue);
            if ($num > 0) {
                $exist = true;
            }
        }
...
//			$url = array($job['controller'], $job['method'], $params);
            ...
            $url = array($job['controller'], $job['method']);
			$url = implode('/', $url);

			// Execute job
			$this -> _ci -> mcurl -> add_call('perform_job', 'POST', site_url($url), $params);
